...
const ProductTotal = ({ onAddToCart, stock }) => {
    ...
    const handleCloseModalCarrito = () => {
        setOpenModalCarrito(false); // Cierra el modal
    };
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginY: 2 }}>
            ...
            <Dialog open={openModalCarrito} onClose={handleCloseModalCarrito}>
                <DialogTitle>Producto agregado al carrito.</DialogTitle>
                <DialogActions>
                    <Button onClick={handleCloseModalCarrito} color="primary">
                        Cerrar
                    </Button>
                </DialogActions>
            </Dialog>
